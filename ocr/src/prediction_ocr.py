...
def get_seg_dict(predictor, on_im, save_dir, fname, THING_CLASSES):
	...
	print (mask.shape)
	boxes = dic['_fields']['pred_boxes'].tensor.cpu().detach().numpy()
	points_list = []
	for objects_num in range(0,mask.shape[0]):
		try:
			box = boxes[objects_num]
			...
			points_list.append((obj_name,box,points))

			print (obj_name,box,points)
...
get_im_seg_info("/home/itis/jaipur_new_las/PANO/Track_A.Ladybug/", predictor, "store/data/Allowed_Classes/prediction3/", THING_CLASSES, ext='jpg')
